...
X.train <- sparse.model.matrix(Item_Outlet_Sales ~ ., data = check.train)[, -1]
...
```{r, echo=TRUE}
best.xgb.cv.fit <- xgb.cv(data = X.train, label = y.train,
...
plot(best.xgb.cv.fit$evaluation_log[1:50, c(1,2)])
