...
train.data_v3 = train.data_v2 %>% 
  ...
  merge(train.data_v2, ., by=c("Outlet_Identifier", "Item_Type")) %>% 
  ungroup() %>% 
...
``` {r}
ggplot(train.data_v3[1:nrow(train.data_v3),], aes(as.factor(Years_Operating), Item_Outlet_Sales)) +
  geom_boxplot() +
  theme_gray() 
...
```{r}
ggplot(train.data_v3, aes(Item_Type, Item_Visibility_v2, fill = Outlet_Size)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = "black")) + 
  xlab("Item Type") + 
...
```{r}
ggplot(train.data_v3, aes(Outlet_Identifier, Item_Visibility_v2)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = "black")) + 
...
```{r}
ggplot() + 
geom_density(aes(x=Item_Visibility_v2), colour="red", data=train.data_v3)

```
```{r}
ggplot(train.data_v3[1:nrow(train.data_v3),], aes(Item_Visibility_v2, Item_Outlet_Sales)) +
