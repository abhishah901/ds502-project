...
In this section, we fit a Principal Component Regression model. First, we start by find the number of principal components needed to maximize the variance explanation and minimizing the MSE while keeping a reasonable number of components. In our case, the number of Principal Components chosen could be `27` as shown on the graphs below: 
...
plot(pcr.fit$Xvar/sum(pcr.fit$Xvar), xlab = 'Number of Principal Components', ylab='Variance unexplained')
...
```{r}
train.pca <- pca.out$x
aside.test.pca <- predict(pca.out, newdata= aside.test.data)
```
