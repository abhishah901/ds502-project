...
One of the most obvious relashionships are between Maximum Retail Price or MRP of the items and their sales.
So we observe that there are 4 ranges of MRP accross all the items
...
```
We also try to plot the sales against the type of outlet colored based on the outlet size. We get some interesting observations which were explained in the presentation.
```{r outlet-sales}
...
```
Next are the sales of each of the 10 outlets. The intuition behind the plot was to observe which outlets perform well and which do not.
...
knitr::kable(data.frame(selected_features = names(coef(backwardSubset,9))[-1]), caption='Forward Selection')
```
...
plot(myLambda,errors,type='l', xlab='Lambda', ylab='Error')+
...
myForest = randomForest(formula=Item_Outlet_Sales~., data=train.rf, do.trace=T, ntree=30)

summary(myForest)

preds <- predict(myForest, newdata= train.rf)

sqrt(mean((train.rf$Item_Outlet_Sales - preds)^2))
```
