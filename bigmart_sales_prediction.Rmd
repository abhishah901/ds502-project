...
train.data_v2 = train.data %>% 
  ...
  rowwise() %>% 
  mutate(Item_Fat_Content_v2 = ifelse(Item_Cat=="NC","not_edible",Item_Fat_Content)) %>% 
  select(-c(Item_Fat_Content,Outlet_Establishment_Year)) 

train.data_v3 = train.data_v2 %>% 
  filter(Item_Visibility>0) %>% 
  group_by(Item_Identifier) %>% 
  summarise(Item_Visibility_Avg = mean(Item_Visibility)) %>% 
  merge(train.data_v2,.,by.x="Item_Identifier",by.y="Item_Identifier") %>% 
  rowwise() %>% 
  mutate(Item_Visibility_v2 = ifelse(Item_Visibility==0,Item_Visibility_Avg,Item_Visibility) ) 
  
train.data_v3 = train.data_v3 %>% 
  mutate(Item_Visibility_v2_Ratio = Item_Visibility_v2/Item_Visibility_Avg ) %>% 
  select(-c(Item_Visibility,Item_Visibility_Avg)) 
...
test.data_v3 = test.data_v2 %>% 
  ...
  group_by(Item_Identifier) %>% 
  summarise(Item_Visibility_Avg = mean(Item_Visibility)) %>% 
  merge(test.data_v2,.,by.x="Item_Identifier",by.y="Item_Identifier") %>% 
  rowwise() %>% 
  mutate(Item_Visibility_v2 = ifelse(Item_Visibility==0,Item_Visibility_Avg,Item_Visibility) ) 
...
holder.Year = data.table(train.data)[,sum(Item_Outlet_Sales), by = Outlet_Establishment_Year]
barplot(setNames(holder.Year$V1,holder.Year$Outlet_Identifier),width = 2, names.arg = unique(train.data$Outlet_Establishment_Year))
...
```{r}
ggplot(train.data, aes(Item_Type, Item_Visibility, fill = Outlet_Size)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = "black")) + 
  xlab("Item Type") + 
...
```{r}
ggplot(train.data, aes(Outlet_Identifier, Item_Visibility)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5, color = "black")) + 
...
```{r}
ggplot() + 
geom_density(aes(x=Item_Visibility), colour="red", data=train.data)
```
```{r}
ggplot(train.data[1:nrow(train.data),], aes(Item_Visibility, Item_Outlet_Sales)) +
