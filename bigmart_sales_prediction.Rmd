...
```{r}
unique_outs <- unique(train.data.mutate %>% select(starts_with("Outlet_")))
unique_outs[,c(1,3)]
```
...
for (i in na_indices) {
  ...
  train.data.mutate[train.data.mutate$Outlet_Identifier == train_num[i,]$Outlet_Identifier,]$Outlet_Size <- max_out_size
}
```
